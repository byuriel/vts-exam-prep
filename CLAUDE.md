...
7. **Supabase cloud autosave** — `cloudSave()` / `cloudLoad()`. Uses the `progress` table, row id `exam_main`. Falls back to `localStorage` (key `vts_exam_prep_v3`) then in-memory if offline. (Shares the Supabase project with VTS Compass but uses a **different row id**, so they don't collide.)
